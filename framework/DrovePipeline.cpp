...
namespace Drove {
    ...
    Pipeline::Pipeline(
        ...
        getWindowExtent(&height, &width);

        this->device=device;

        std::vector<char> fragShaderCode = readFile(fragPath);
        std::vector<char> vertShaderCode = readFile(vertPath);
        ...
        viewportState.pViewports = &viewport; // not necessary w dyn state
        viewportState.scissorCount = 1;
        viewportState.pScissors = &scissor; // not necessary w dyn state
